...
public:
    ...
    explicit Sine3DFunction(const float &ya, const float &yb, const float &yc, const float &xa, const float &xb, const float &xc, const float &d, const Vec3 &origin, const Material &material)
        : RObj(origin, material), ya(ya), yb(yb), yc(yc), xa(xa), xb(xb), xc(xc), d(d) {}

    HitResult hit(const Ray& ray) const override {
        Vec3 O = ray.origin - origin;
        Vec3 r = O + ray.direction * t;
        0 = xa*sin(xb*x + xc) + ya*sin(yb*y+yc) +d-z;
